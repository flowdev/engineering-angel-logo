...
func main() {
	...
	// wing2
	wing2ClipX := parenClipX + parenClipWidth
	wing2ClipY := margin - stroke/2
	wing2ClipWidth := oSize + stroke/2
	wing2ClipHeight := oSize + stroke/2
	wing2CircX := wing2ClipX
	wing2CircY := wing2ClipY + wing1ClipHeight
	wing2CircR := oSize

	// all
	widthAll := wing2ClipX + wing2ClipWidth + margin
	...
	fmt.Printf(`    <rect x="%d" y="%d" width="%d" height="%d" />`,
		wing2ClipX, wing2ClipY, wing2ClipWidth, wing2ClipHeight)
	fmt.Println()
	fmt.Println(`  </clipPath>`)
	fmt.Printf(`  <circle clip-path="url(#wing2Clip)" cx="%d" cy="%d" r="%d" />`,
		wing2CircX, wing2CircY, wing2CircR)
	fmt.Println()
	featherR = (wing2CircR - stroke) / featherN / 2
	feather0X = wing2ClipX + wing2ClipWidth - stroke/2
	feather0Y = wing2CircY + oSize - featherR
	fmt.Printf(`  <line x1="%d" y1="%d" x2="%d" y2="%d" />`,
		feather0X, wing2CircY, feather0X, feather0Y)
	fmt.Println()
	for i := 0; i < featherN; i++ {
		featherX := feather0X - featherR - i*featherR*2
		featherY := feather0Y - i*featherR*2
